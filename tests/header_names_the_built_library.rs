...
//!
//! `include/vmdk.h` said "Link with libam_img_vmdk.a" for as long as the file
//! existed, and cargo has never produced that name: `[lib] name` is
//! `vmdk`, so the artefact is `libvmdk.a` — which is exactly what
...
//! lines, and rejected three spellings cargo accepts: `name = 'vmdk'`
//! in single quotes, `name = "vmdk" # comment` with a trailing
...
/// `Vmdk` produced `libVmdk.a`, the header's correct `Link with
/// libVmdk.a` was lowered to `libvmdk.a`, and the guard REJECTED a
...
/// saying `libvmdk.a` while cargo builds `libVmdk.a` would pass — and
...
fn the_lib_name_is_parsed_rather_than_scanned() {
    ...
    let trailing_comment =
        "[package]\nname = \"am-img-vmdk\"\n\n[lib]\nname = \"vmdk\" # the exported ABI name\n";
...
fn the_lib_name_comes_from_the_lib_section_and_not_the_package() {
    ...
    assert_eq!(
        lib_name(toml).as_deref(),
        Some("vmdk"),
        "using the package name would look for libam-img-vmdk.a"
    );
...
/// libvmdk.a` satisfied a check for "does the header say to link it".
///
...
fn the_link_instruction_must_open_the_sentence() {
    ...
    for accepted in [
        " * Link with libvmdk.a alongside fs_core.h.\n",
        "Link with libvmdk.a\n",
        "// Link with libvmdk.a and include this header.\n",
        "  # link with   libvmdk.a\n",
        " */ Link with libvmdk.a\n",
    ...
    for refused in [
        // The filed defect.
        " * Do not link with libvmdk.a; it is an implementation detail.\n",
        " * You must never link with libvmdk.a directly.\n",
        // A mention with no instruction: what rust-img-vmdk#76's first fix caught.
        " * The build produces libvmdk.a in the target directory.\n",
...
/// declaring `[lib] name = "Vmdk"` had its correct `Link with
/// libVmdk.a` rejected — in the packaging step, after a green CI.
...
fn a_mixed_case_library_name_is_read_as_itself() {
    assert!(
        states_the_link_instruction(" * Link with libVmdk.a alongside fs_core.h.\n", "libVmdk.a"),
    ...
    for spelling in [
        ...
        assert!(
            states_the_link_instruction(spelling, "libvmdk.a"),
            "{spelling:?} is the instruction; only its case differs"
        );
    }

    assert!(
        !states_the_link_instruction(" * Link with libvmdk.a\n", "libVmdk.a"),
        "cargo builds libVmdk.a and the header says libvmdk.a. A linker is \
...
fn the_header_scan_finds_library_names_in_prose() {
    let header = " * Link with libvmdk.a and include this header alongside fs_core.h.\n";
    assert_eq!(libraries_named(header), vec!["libvmdk.a"]);

    // The exact defect, and the near-miss that has to stay distinct.
    assert_eq!(
        libraries_named(" * Link with libam_img_vmdk.a and include this\n"),
        vec!["libam_img_vmdk.a"]
    );
