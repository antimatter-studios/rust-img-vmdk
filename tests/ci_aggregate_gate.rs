//! One check gates a pull request, and it stands for every job (#111).
//!
//! Branch protection names checks, and a check is a job name. This
//! repository named six of them by hand — `fmt`, the three `test / <os>`
//! matrix legs, `test (release)` and `qemu-validation` — so the list had to
//! be edited whenever a job was renamed, split into a matrix leg or added,
//! and until someone did, the new work was required by nobody. The opposite
//! spelling is worse: a required check no job produces reads as permanently
//! PENDING rather than failed, and with `enforce_admins` on nothing merges
//! and there is no failure to point at. `test / release` for
//! `test (release)` is all it takes.
//!
//! So protection names one job, `ci-ok`, which `needs:` every other job in
//! ci.yml and fails unless each concluded success. This is what keeps that
//! true, because nothing else would: every job in `ci.yml` has to be in its
//! `needs`, `.github-guard` has to require it and nothing else, and it has
//! to carry `if: always()` so a skipped or cancelled job is judged instead
//! of leaving the aggregate skipped beside it.
//!
//! `needs:` reaches jobs in ci.yml alone, which is the property that makes
//! this safe here: release.yml has its own `test` and `qemu-validation`
//! jobs, on a `v*.*.*` tag, and neither they nor fuzz.yml's nightly job can
//! be pulled into the gate by the names they share.
