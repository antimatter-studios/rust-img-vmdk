...
# release tiers run the same targets in two profiles, on purpose, and those
# tests are counted twice. A distinct-test count would be the same number
# whether or not both tiers still ran, and the thing worth noticing here is a
# tier quietly stopping -- which shows up as a drop in this number and in
# nothing else.
#
...
for tier in "$@"; do
    ...
    n="$(grep -aoE 'test result: ok\. [0-9]+ passed' "$log" | awk '{ sum += $4 } END { print sum + 0 }')"
    total=$(( total + n ))
    tiers=$(( tiers + 1 ))
done
